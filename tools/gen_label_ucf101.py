...
root = r"D:\保研\论文\UCF101\picture"     # 抽帧后的图片存放目录文件夹

def parse_ucf_splits():
    class_ind = [x.strip().split() for x in open(r"D:\保研\论文\UCF101\UCF101TrainTestSplits-RecognitionTask\ucfTrainTestlist\classInd.txt")]
    ...
    for i in range(1, 4):
        # 修正：删除多余的 .txt
        train_list = [line2rec(x) for x in open(r"D:\保研\论文\UCF101\UCF101TrainTestSplits-RecognitionTask\ucfTrainTestlist\trainlist{:02d}.txt".format(i))]
        test_list = [line2rec(x) for x in open(r"D:\保研\论文\UCF101\UCF101TrainTestSplits-RecognitionTask\ucfTrainTestlist\testlist{:02d}.txt".format(i))]
...
f_info = parse_directory(r"D:\保研\论文\UCF101\picture")

out_path = r"D:\保研\论文\UCF101\label"
dataset = "ucf101"
